-- ============================================================================
-- 1) Fill Tamil + English meanings for every morpheme (individual box) and
--    every combined word (combined box) of the 21-line poem, replacing the
--    '[பொருள்]' / '[பொருள் சேர்க்கவும்]' placeholders.
-- 2) Rename the topic "என் அம்மை" -> "யுகத்தின் பாடல்" and move it to the
--    top of the செய்யுள் list (order_index lower than every other topic).
--
-- NOTE ON SPELLING: you typed "யுகத்தின பாடல்" — I've used the correct
-- spelling "யுகத்தின் பாடல்" (with the missing ் on ன்), matching the
-- original topic name before it was deleted. Tell me if you actually want
-- different text and I'll fix it.
--
-- Meanings are written as "தமிழ் பொருள்; English gloss" in one line, since
-- the popup shows this as plain text. These are my own linguistic reading
-- of the poem (it reads as an ode to a mother/motherland — farmers who
-- ploughed, sowed and worshipped her feet, receiving abundant grain over
-- countless years) — not sourced from an external dictionary API. If your
-- textbook gives official meanings for any word, tell me and I'll correct
-- that specific row; this is meant to get every popup populated with a
-- reasonable, correct-sounding meaning so nothing is blank, not to replace
-- your own final review.
--
-- Safe to re-run.
-- ============================================================================
...
BEGIN
  ...
  WHERE t.title = 'என் அம்மை' AND p.page_type = 'செய்யுள் பகுதி'
  LIMIT 1;

  IF v_page_id IS NULL THEN
    RAISE EXCEPTION 'என் அம்மை செய்யுள் பகுதி page not found — run en_ammai_full_setup.sql first.';
  END IF;
...
-- ============================================================================
-- Rename topic + move to top of the செய்யுள் list
-- ============================================================================
DO $$
DECLARE
  v_min_order int;
BEGIN
  SELECT MIN(t.order_index) INTO v_min_order
  FROM topics t JOIN sections s ON s.id = t.section_id
  WHERE s.name = 'செய்யுள்';

  UPDATE topics
  SET title = 'யுகத்தின் பாடல்',
      order_index = COALESCE(v_min_order, 10) - 10
  WHERE title = 'என் அம்மை';

  RAISE NOTICE '✓ Topic renamed to யுகத்தின் பாடல் and moved to the top (order_index=%).', COALESCE(v_min_order, 10) - 10;
END $$;

-- ── Verify ─────────────────────────────────────────────────────────────────
SELECT title, order_index FROM topics
JOIN sections s ON s.id = topics.section_id
WHERE s.name = 'செய்யுள்'
