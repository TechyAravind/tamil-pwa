...
export default function QuestionFeedback({
  ...
  showExplanation, setShowExplanation, onNext, gaveUp, nextLabel = 'Next →',
}) {
  ...
  return (
    <div className="mt-4 space-y-3">
      <p className={`font-bold text-base ${gaveUp ? 'text-gray-500' : correct ? 'text-green-600' : 'text-red-600'}`}>
        {gaveUp ? 'Here’s the answer:' : correct ? 'Correct!' : 'Not quite.'}
      </p>
      ...
      {showExplanation && explanationText && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-2">
          <p className="font-bold text-sm text-gray-800">Solution</p>
          <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{explanationText}</p>
          {commonMistake && (
            <div className="border-l-4 border-amber-400 bg-amber-50 pl-3 pr-2 py-2 rounded-r-lg">
              <p className="text-xs text-amber-800 leading-relaxed">
                <span className="font-bold">Common mistake: </span>{commonMistake}
              </p>
            </div>
          )}
        </div>
